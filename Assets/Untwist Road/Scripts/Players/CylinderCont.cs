...
public class CylinderCont : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider target)
    {
        ...
        if (target.tag == "GroundOff")
        {
            ...
            playerrb.AddForce(0, 2f, 0, ForceMode.Impulse); //Add force to player
        }
    }
    ...
    void DeadInGround() //Dead on collision with a conventional obstacle
    {
        ...
        playerrb.AddForce(new Vector3(0,3,2), ForceMode.Impulse); //Add force
    ...
    void DeadInRoad() //Dead when the cylinders run out when spawning
    {
        ...
        playerrb.AddForce(new Vector3(0, 7f, 2f), ForceMode.Impulse); //Add force
